...
async function readKitVersion(kitRoot: string): Promise<string> {
...
async function resolveConflictActions(conflicts: string[]): Promise<Set<string>> {
  const skipPaths = new Set<string>();
  if (conflicts.length === 0) return skipPaths;
...
export async function runInit(options: InitOptions): Promise<void> {
  ...
  if (options.global) {
    ...
    if (conflicts.length > 0 && skipPaths.size < conflicts.length) {
      ...
      if (backup) console.log(`Backup created at ${backup}`);
    }
  ...
  await fs.writeFile(
    path.join(targetRoot, ".claude", ".ck.json"),
    JSON.stringify({ version, kit: options.kit }, null, 2) + "\n",
    "utf8"
  );
