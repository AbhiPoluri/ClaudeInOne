...
export async function runVersions(): Promise<void> {
  const filePath = fileURLToPath(import.meta.url);
  const cliRoot = path.resolve(path.dirname(filePath), "..", "..");

  const pkg = await readJson<{ version?: string }>(path.join(cliRoot, "package.json"));
  const kit = await readJson<{ version?: string }>(path.join(cliRoot, "kit", ".claude", ".ck.json"));
